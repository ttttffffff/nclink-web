  ...
  <div class="customer">
    ...
    <Form :model="formInfo" :label-width="100" style="padding: 20px 0 0px;margin-bottom: 5px; width: 1000px;">
     ...
     <row>
      ...
      <Col span="12">
        <form-item label="OutFile路径">
         <Input type="text" v-model="formInfo.OutFileInfo" placeholder="请输入需要写入的文件路径" style="width: 300px;"/>
       </form-item>
      </Col>
     </row>
     <row>
     ...
     <row>
      ...
      <Form-item>
        ...
        <Button type="plain"  @click="showOutFile" style="margin-left: 10px;">{{ showSub?'收起日志信息':'查看日志' }}</Button>
       </Form-item>
      </div>
     </row>
   </Form>
    <Input 
      ...
      v-show="showSub" 
      readonly 
      style="margin-top: 5px;width:900px; white-space:pre-wrap"/>
  ...
  export default {
    ...
    methods: {
      showOutFile(){
        ...
        if(this.showSub){
          axios.post('/backend/sub/info',{
            "FilePath": this.formInfo.OutFile,
          }).then(response => {
        ...
        // this.SubCotent=reader.result
        // this.getLogSuccess=true
        this.SubCotent=response.data['Msg']
        }).catch(error => {
          this.$Message.error('error: ' + error.data.Msg)
          this.loading = false
      })
        }
      ...
      submitForm(){
        console.log('发送sub信息')
        axios.post('/backend/sub',this.formInfo).then(response => {
          if (response.data['Code'] !== 0) {
            throw response
          }
